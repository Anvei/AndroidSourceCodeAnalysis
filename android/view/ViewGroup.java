...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    public static int getChildMeasureSpec(int spec, int padding, int childDimension) {
        ...
        int size = Math.max(0, specSize - padding);     // 父容器的size减去padding即子View可用空间
                                                        // size表示的是子View全部可用空间，即match_parent对应的大小
        int resultSize = 0;
        int resultMode = 0;

        switch (specMode) {
        // Parent has imposed an exact size on us
        // 根据父容器的specMode来确定子View的大小
        case MeasureSpec.EXACTLY:                       // 对应于具体的数值或者match_parent，即对父容器已知
            if (childDimension >= 0) {
                resultSize = childDimension;
                resultMode = MeasureSpec.EXACTLY;
            } else if (childDimension == LayoutParams.MATCH_PARENT) {   // LayoutParams的几个参数都是小于0的
